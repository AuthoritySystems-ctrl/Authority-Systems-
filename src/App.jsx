...
const PinLogin = ({ onLogin }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.purpleDark, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, gap: 20 }}>
      <img src={"/logo.png"} alt="Authority Systems" style={{ width: 150, height: "auto", borderRadius: 20, boxShadow: "0 8px 30px rgba(212,160,23,0.25)", border: `2px solid ${C.gold}` }} />
      <Logo size={30} />
      <div style={{ width: "100%", maxWidth: 320, marginTop: 4 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, maxWidth: 230, margin: "0 auto 10px" }}>
          {[1,2,3,4,5,6,7,8,9,"",0,"DEL"].map((k, i) => (
            <button key={i} onClick={() => { if (k === "DEL") setPin(p => p.slice(0,-1)); else if (k !== "" && pin.length < 4) setPin(p => p + k); }} style={{ height: 50, borderRadius: 10, border: k === "" ? "none" : `1px solid ${C.purpleLight}`, background: k === "" ? "transparent" : C.purple, color: C.gold, fontSize: 20, fontWeight: 700, cursor: k === "" ? "default" : "pointer" }}>{k}</button>
          ))}
        </div>
            ...
            return (
              <button key={side.id} onClick={() => toggle(side.id)} disabled={out} style={{ background: picked ? C.gold : out ? "#1a0a18" : C.purple, color: picked ? C.purple : out ? C.gray500 : C.goldPale, border: `2px solid ${picked ? C.gold : out ? C.red : low ? C.orange : C.purpleLight}`, borderRadius: 10, padding: "12px 14px", cursor: out ? "not-allowed" : "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, background: out ? C.red : low ? C.orange : "transparent", color: out || low ? C.white : picked ? C.gold : "transparent", padding: out || low ? "2px 8px" : 0, borderRadius: 8 }}>
                  {out ? "OUT OF STOCK" : low ? `Only ${side.stock} left` : picked ? "✔️" : ""}
                </span>
              </button>
            );
          })}
...
const TransferModal = ({ table, managerPin, onConfirm, onCancel }) => {
  ...
  useEffect(() => {
    try { setHasBiometric(!!localStorage.getItem("authorityBiometric")); } catch (e) {}
  }, []);

  const currentWaiter = STAFF.find(s => s.id === table.waiterId);
  const waiterOptions = STAFF.filter(s => s.role === "waiter" && s.id !== table.waiterId);
  const label = table.isTakeaway ? table.takeawayNumber : `Table ${table.id}`;

  const confirmPin = () => {
    if (pin === managerPin) {
      onConfirm(newWaiterId);
    } else {
  ...
  const confirmBiometric = async () => {
    ...
    try {
      ...
      if (assertion) onConfirm(newWaiterId);
    } catch (e) {
      setError("Fingerprint failed — try PIN instead");
    }
    setBioBusy(false);
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 20 }}>
      <div style={{ background: C.purple, borderRadius: 16, padding: 24, width: "100%", maxWidth: 340, border: `2px solid ${C.gold}` }}>
        {step === "select" ? (
          <>
            <h3 style={{ color: C.gold, margin: "0 0 4px" }}>Transfer {label}</h3>
            <p style={{ color: C.goldPale, fontSize: 12, marginBottom: 16 }}>Currently: {currentWaiter ? currentWaiter.name : "Unassigned"}</p>
            <p style={{ color: C.goldPale, fontSize: 13, marginBottom: 8 }}>Reassign to</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18, maxHeight: 220, overflowY: "auto" }}>
              {waiterOptions.map(s => (
                <button key={s.id} onClick={() => setNewWaiterId(s.id)} style={{ padding: "10px 14px", borderRadius: 8, border: `2px solid ${newWaiterId === s.id ? C.gold : C.purpleLight}`, background: newWaiterId === s.id ? C.gold : "transparent", color: newWaiterId === s.id ? C.purple : C.goldPale, fontWeight: 700, textAlign: "left", cursor: "pointer" }}>{s.name}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={onCancel} color={C.purpleLight} textColor={C.goldPale} style={{ flex: 1 }}>Cancel</Btn>
              <Btn onClick={() => setStep("auth")} disabled={!newWaiterId} style={{ flex: 1 }}>Next ></Btn>
            </div>
          </>
        ) : (
          <>
            <h3 style={{ color: C.gold, margin: "0 0 4px" }}>Manager Authorization</h3>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 14 }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ width: 40, height: 40, borderRadius: 8, background: C.purpleDark, border: `2px solid ${pin.length > i ? C.gold : C.purpleLight}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.gold, fontSize: 18, fontWeight: 900 }}>
                  {pin.length > i ? "●" : ""}
                </div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, maxWidth: 220, margin: "0 auto 14px" }}>
              {[1,2,3,4,5,6,7,8,9,"",0,"DEL"].map((k, i) => (
                <button key={i} onClick={() => { if (k === "DEL") setPin(p => p.slice(0,-1)); else if (k !== "" && pin.length < 4) setPin(p => p + k); }} style={{ height: 42, borderRadius: 8, border: k === "" ? "none" : `1px solid ${C.purpleLight}`, background: k === "" ? "transparent" : C.purpleDark, color: C.gold, fontSize: 16, fontWeight: 700, cursor: k === "" ? "default" : "pointer" }}>{k}</button>
              ))}
            </div>
            {error && <p style={{ color: C.redLight, fontSize: 12, textAlign: "center", marginBottom: 10 }}>{error}</p>}
            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={() => { setStep("select"); setPin(""); setError(""); }} color={C.purpleLight} textColor={C.goldPale} style={{ flex: 1 }}>Back</Btn>
              <Btn onClick={confirmPin} disabled={pin.length !== 4} style={{ flex: 1 }}>Confirm Transfer</Btn>
            </div>
          </>
        )}
      </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {pending.map(t => (
              <div key={t.id} style={{ background: t.isTakeaway ? C.teal : C.purple, borderRadius: 12, padding: 16, border: `2px solid ${orderAgeColor(t.orderSentTs) || C.gold}`, marginBottom: 12 }}>
                ...
                <Btn onClick={() => markReady(t.id)} color={C.greenLight} textColor={C.white} style={{ marginTop: 10, width: "100%" }}>✔️ Mark Ready</Btn>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ marginBottom: 20 }}>
            {stockRequests.map(req => (
              <div key={req.id} style={{ background: C.purple, borderRadius: 12, padding: 16, border: `2px solid ${C.orange}`, marginBottom: 12 }}>
                ...
                <Btn onClick={() => applyStockRequest(req)} color={C.greenLight} textColor={C.white} style={{ marginTop: 10, width: "100%" }}>✔️ Mark Updated</Btn>
              </div>
            ))}
          </div>
        )}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 20 }}>
      <div style={{ background: C.purple, borderRadius: 16, padding: 22, width: "100%", maxWidth: 360, maxHeight: "85vh", overflowY: "auto", border: `2px solid ${C.gold}` }}>
        ...
        <button onClick={() => setHasSides(v => !v)} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: "pointer", marginBottom: hasSides ? 10 : 18, padding: 0 }}>
          <div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${C.gold}`, background: hasSides ? C.gold : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: C.purple, fontWeight: 900, fontSize: 13 }}>{hasSides ? "✔️" : ""}</div>
...
const FloorPlan = ({ tables, setTables, canReserve, canTransfer = false, addNotification, menu, sides, setMenu, setSides, user }) => {
  ...
  const transferTable = transferTableId ? tables.find(t => t.id === transferTableId) : null;

  const doTransfer = (newWaiterId) => {
    setTables(prev => prev.map(t => t.id === transferTableId ? { ...t, waiterId: newWaiterId } : t));
    const t = tables.find(x => x.id === transferTableId);
    const label = t?.isTakeaway ? t.takeawayNumber : `Table ${transferTableId}`;
    const newStaff = STAFF.find(s => s.id === newWaiterId);
    if (user) {
      supabase.from("shift_events").insert({
        staff_id: user.id, staff_name: user.name, role: "manager",
        event_type: "table_transfer", details: { label, to: newStaff?.name },
      });
    }
    addNotification && addNotification(`${label} transferred to ${newStaff?.name}`);
        ...
        <div style={{ background: C.purple, borderRadius: 12, padding: 16, border: `2px solid ${C.gold}`, marginBottom: 16 }}>
          ...
          {selTable.status === "occupied" && (
            <div>
              ...
              <div style={{ color: C.goldPale, fontSize: 13, marginBottom: 4 }}>Guests: <strong style={{ color: C.gold }}>{selTable.guests}</strong></div>
              <div style={{ color: C.goldPale, fontSize: 13, marginBottom: 10 }}>Waiter: <strong style={{ color: C.gold }}>{STAFF.find(s => s.id === selTable.waiterId)?.name || "Unassigned"}</strong></div>
              ...
              {canTransfer && <Btn onClick={() => setTransferTableId(selTable.id)} color={C.purpleLight} textColor={C.goldPale} style={{ width: "100%", marginTop: 12 }}>⇄ Transfer to Another Staff</Btn>}
            </div>
          )}
          {selTable.status === "bill" && (
            <div>
              ...
              <div style={{ color: C.goldPale, fontSize: 13, margin: "10px 0" }}>Waiter: <strong style={{ color: C.gold }}>{STAFF.find(s => s.id === selTable.waiterId)?.name || "Unassigned"}</strong></div>
              {canTransfer && <Btn onClick={() => setTransferTableId(selTable.id)} color={C.purpleLight} textColor={C.goldPale} style={{ width: "100%" }}>⇄ Transfer to Another Staff</Btn>}
            </div>
          )}
        </div>
      )}
      {transferTable && <TransferModal table={transferTable} managerPin={user?.pin} onConfirm={doTransfer} onCancel={() => setTransferTableId(null)} />}
      {reserveModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 20 }}>
          <div style={{ background: C.purple, borderRadius: 16, padding: 24, width: "100%", maxWidth: 340, border: `2px solid ${C.gold}` }}>
...
const ManagerView = ({ tables, setTables, menu, setMenu, sides, setSides, user, addNotification }) => {
  ...
  const doTransfer = (newWaiterId) => {
    ...
    addNotification(`${label} transferred to ${newStaff?.name}`);
    setTransferTableId(null);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      ...
      <div style={{ padding: 16 }}>
        {tab === "floor" && <FloorPlan tables={tables} setTables={setTables} canReserve={true} canTransfer={true} addNotification={addNotification} menu={menu} sides={sides} setMenu={setMenu} setSides={setSides} user={user} />}
        {tab === "tables" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <div>
            ...
            {menu.map(item => (
              <button key={item.id} onClick={() => setEditingItem(item)} style={{ width: "100%", textAlign: "left", background: C.purple, borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, border: `1px solid ${C.purpleLight}`, cursor: "pointer" }}>
                ...
                <span style={{ color: C.gold, fontSize: 16 }}>✏️</span>
              </button>
            ))}
      ...
      {transferTableId && tables.find(t => t.id === transferTableId) && (
        <TransferModal table={tables.find(t => t.id === transferTableId)} managerPin={user?.pin} onConfirm={doTransfer} onCancel={() => setTransferTableId(null)} />
      )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400, padding: 20 }}>
          <div style={{ background: C.purpleDark, borderRadius: 16, padding: 20, width: "100%", maxWidth: 420, maxHeight: "80vh", overflowY: "auto", border: `2px solid ${C.gold}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
              ...
              <button onClick={() => setSelectedShift(null)} style={{ background: "none", border: "none", color: C.gold, fontSize: 22, cursor: "pointer" }}>X</button>
            </div>
