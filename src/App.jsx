...
const CATEGORIES = ["Starters", "Mains", "Desserts", "Drinks"];// ── Shared UI ─────────────────────────────────────────────────
const Logo = ({ size = 32 }) => (
...
const Notification = ({ msg, onClose }) => (
  <div style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", zIndex: 9999, background: C.gold, color: C.purple, borderRadius: 10, padding: "12px 20px", fontWeight: 700, fontSize: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.4)", display: "flex", alignItems: "center", gap: 12, maxWidth: 340, width: "90%" }}>
    <span>🔔 {msg}</span>
    <span onClick={onClose} style={{ cursor: "pointer", fontWeight: 900, marginLeft: "auto" }}>×</span>
  </div>
);
...
// ── Open Table Modal ──────────────────────────────────────────
const OpenTableModal = ({ tableId, tables, onOpen, onCancel }) => {
...
// ── Order Panel (reusable by waiter AND manager) ──────────────
const OrderPanel = ({ activeTable, setActiveTable, tables, setTables, menu, sides, userId, addNotification, onBack, showBillButton = true }) => {
  ...
  const sendToKitchen = () => {
    ...
    addNotification(`Table ${activeTable.id} order sent to kitchen`);
    onBack();
  };
  ...
  const total = orderTotal(activeTable.order);
    ...
    <div style={{ minHeight: "100vh", background: C.purpleDark, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: C.purple, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 100 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: C.gold, fontSize: 22, cursor: "pointer" }}>←</button>
        <div style={{ flex: 1 }}>
          <div style={{ color: C.gold, fontWeight: 800 }}>Table {activeTable.id}</div>
          <div style={{ color: C.goldPale, fontSize: 11 }}>{activeTable.guests} guests · Opened {activeTable.openedAt}</div>
        </div>
        <Logo size={22} />
      </div>
      ...
      <div style={{ background: C.purple, borderTop: `2px solid ${C.gold}`, padding: 14, position: "sticky", bottom: 0 }}>
            ...
            <div style={{ maxHeight: 130, overflowY: "auto", marginBottom: 10 }}>
              {activeTable.order.map(o => (
                <div key={o.orderKey} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div style={{ flex: 1 }}>
                    <span style={{ color: C.goldPale, fontWeight: 700, fontSize: 12 }}>{o.qty}× {o.name}</span>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <button onClick={() => removeLine(o.orderKey)} style={{ background: "none", border: "none", color: C.redLight, fontWeight: 900, fontSize: 16, cursor: "pointer", padding: 0 }}>×</button>
                  </div>
                </div>
              ))}
            </div>
...
const WaiterView = ({ tables, setTables, menu, sides, user, addNotification }) => {
  ...
  if (view === "order" && activeTable) {
    return (
      ...
      />
    );if (view === "bill" && activeTable) return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      <div style={{ background: C.purple, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
        <button onClick={() => setView("tables")} style={{ background: "none", border: "none", color: C.gold, fontSize: 22, cursor: "pointer" }}>←</button>
        <div style={{ color: C.gold, fontWeight: 800 }}>Table {activeTable.id} — Bill</div>
      </div>
      <div style={{ padding: 16 }}>
        <div style={{ background: C.purple, borderRadius: 12, padding: 20, border: `1px solid ${C.gold}`, marginBottom: 16 }}>
          <div style={{ color: C.gold, fontWeight: 800, textAlign: "center", marginBottom: 14, fontSize: 15 }}>AUTHORITY SYSTEMS</div>
          {activeTable.order.map((o, i) => (
            <div key={i} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px dotted ${C.purpleLight}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", color: C.goldPale, fontSize: 14 }}>
                <span style={{ fontWeight: 700 }}>{o.qty}× {o.name}</span>
                <span style={{ color: C.gold }}>{fmt(o.price * o.qty)}</span>
              </div>
              {o.sides && <div style={{ color: C.gray500, fontSize: 11, marginTop: 2 }}>↳ {o.sides.map(s => s.name).join(", ")}</div>}
            </div>
          ))}
          <div style={{ display: "flex", justifyContent: "space-between", color: C.gold, fontWeight: 900, fontSize: 20, marginTop: 10 }}>
            <span>TOTAL</span><span>{fmt(orderTotal(activeTable.order))}</span>
          </div>
        </div>
        <Btn onClick={() => setView("order")} color={C.purpleLight} textColor={C.goldPale} style={{ width: "100%" }}>+ Add More Items</Btn>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      <TopBar user={user} />
      {openingTable && <OpenTableModal tableId={openingTable} tables={tables} onOpen={(g) => openTable(openingTable, g)} onCancel={() => setOpeningTable(null)} />}
      <div style={{ padding: 16 }}>
        <h2 style={{ color: C.gold, margin: "0 0 14px", fontSize: 15, letterSpacing: 1 }}>TABLES</h2>
...
const KitchenView = ({ tables, setTables, menu, setMenu, sides, setSides, user, addNotification }) => {
  ...
  const markReady = (tableId) => {
    ...
    addNotification(`Table ${tableId} is ready!`);
  };
  return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      <TopBar user={user} />
      <div style={{ padding: 16 }}>
        ...
        {pending.length === 0
          ...
          : pending.map(t => (
            <div key={t.id} style={{ background: C.purple, borderRadius: 12, padding: 16, border: `2px solid ${C.gold}`, marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ color: C.gold, fontWeight: 800, fontSize: 18 }}>Table {t.id}</div>
                <div style={{ color: C.goldPale, fontSize: 12 }}>Sent {t.orderSentAt}</div>
              </div>
              {t.order.map((o, i) => (
                <div key={i} style={{ marginBottom: 8 }}>
                  <div style={{ color: C.goldPale, fontSize: 14, fontWeight: 700 }}>{o.qty}× {o.name}</div>
...
const CashierView = ({ tables, setTables, user }) => {
  ...
  const processPay = (tableId) => {
    setTables(prev => prev.map(t => t.id === tableId ? { ...t, status: "free", guests: 0, waiterId: null, order: [], orderSentAt: null, openedAt: null, reservation: null } : t));
  ...
  if (selTable) return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      <div style={{ background: C.purple, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: C.gold, fontWeight: 800, fontSize: 16 }}>Table {selTable.id}</div>
          <div style={{ color: C.goldPale, fontSize: 11 }}>{selTable.guests} guests · Opened {selTable.openedAt}</div>
        </div>
        ...
        <div style={{ background: C.purple, borderRadius: 12, padding: 20, border: `1px solid ${C.gold}`, marginBottom: 20 }}>
          ...
          {selTable.order.map((o, i) => (
            <div key={i} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: `1px dotted ${C.purpleLight}` }}>
              ...
              <div style={{ color: C.gray500, fontSize: 11, marginTop: 2 }}>{fmt(o.price)} each</div>
            </div>
          ))}
    ...
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {billTables.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {billTables.map(t => (
              <button key={t.id} onClick={() => setSelectedTable(t.id)} style={{ background: C.purple, borderRadius: 12, padding: "16px 18px", border: `2px solid ${C.orange}`, display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", textAlign: "left" }}>
                <div>
                  <div style={{ color: C.gold, fontWeight: 800, fontSize: 18 }}>Table {t.id}</div>
                  <div style={{ color: C.goldPale, fontSize: 12, marginTop: 3 }}>{t.guests} guests · {t.order.length} item{t.order.length !== 1 ? "s" : ""} · Opened {t.openedAt}</div>
                </div>
                <div style={{ textAlign: "right" }}>
        ...
        <div style={{ background: C.purple, borderRadius: 12, padding: 16, marginTop: 24, border: `1px solid ${C.purpleLight}` }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", color: C.goldPale, fontSize: 13, marginBottom: 6 }}>
            ...
            <span style={{ color: C.orange, fontWeight: 700 }}>{billTables.length}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", color: C.goldPale, fontSize: 13 }}>
            <span>Total outstanding</span>
            <span style={{ color: C.gold, fontWeight: 700 }}>{fmt(billTables.reduce((s, t) => s + orderTotal(t.order), 0))}</span>
          </div>
        </div>
      </div>
    </div>// ── Floor Plan Component (shared) ─────────────────────────────
                ...
                <>
                  {selTable.order.map((o, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", color: C.goldPale, fontSize: 12, marginBottom: 5 }}>
                      <span>{o.qty}× {o.name}{o.sides ? ` (${o.sides.map(s => s.name).join(", ")})` : ""}</span>
                      <span style={{ color: C.gold }}>{fmt(o.price * o.qty)}</span>
                    </div>
                  ))}
...
const ManagerView = ({ tables, setTables, menu, setMenu, sides, user, addNotification }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.purpleDark }}>
      <TopBar user={user} />
      {openingTable && (
        <OpenTableModal tableId={openingTable} tables={tables}
          onOpen={(g) => openTable(openingTable, g)}
      ...
      <div style={{ padding: 16 }}>
        ...
        {tab === "tables" && (
          <div>
            <p style={{ color: C.goldPale, fontSize: 12, marginBottom: 14 }}>Tap a free table to open it, or an active table to manage its order.</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 10 }}>
              {tables.map(t => {
                const free = t.status === "free";
                const reserved = t.status === "reserved";
                ...
                return (
                  <button key={t.id} onClick={() => {
                    if (free || reserved) setOpeningTable(t.id);
                    else if (active) { setActiveTable(tables.find(x => x.id === t.id)); setShowOrder(true); }
                  }} style={{ background: bg, border: `2px solid ${active ? C.gold : (free || reserved) ? C.purpleLight : "transparent"}`, borderRadius: 12, padding: "14px 10px", cursor: "pointer", textAlign: "left" }}>
                    <div style={{ color: C.gold, fontWeight: 800, fontSize: 18 }}>T{t.id}</div>
                    <div style={{ color: C.goldPale, fontSize: 11, marginTop: 3 }}>{free ? "Free" : reserved ? `🔖 ${t.reservation?.name}` : t.status === "bill" ? "⏳ Bill" : `${t.guests} guests`}</div>
